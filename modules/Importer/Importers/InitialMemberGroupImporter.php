 ...
 */
class InitialMemberGroupImporter implements Importer {
    use HasLog, HasImportIdentifier, ReadsSheet;
    protected static $importerIdentifier = "initial-member-group";
    protected $path = 'vend-customer-export.xlsx';
    ...
    public function inserter($row){
        ...
        if($this->getRowData($row, 'HUB CUSTOMER_CODE')){
            $users = \craft\elements\User::find()->yalumbaCustomerId($this->getRowData($row, 'HUB CUSTOMER_CODE'))->all();
            foreach($users as $user){
                $group = $this->getGroup($this->getRowData($row,'CUSTOMER_GROUP_NAME'));
                if($group){
                    \Craft::$app->getUsers()->assignUserToGroups($user->id, [$group->id]);
                }
                $user->setFieldValue('vendCustomerId', $this->getRowData($row, 'VEND ID'));
