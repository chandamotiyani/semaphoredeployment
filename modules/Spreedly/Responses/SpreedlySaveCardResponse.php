...
class SpreedlySaveCardResponse implements RequestResponseInterface{
		...
		if($this->getCode() === "200" || $this->getCode() === "201"){
			if($this->api_response_body->transaction->succeeded == true){
				if($this->api_response_body->transaction->state == "succeeded"){
                    if($this->api_response_body->transaction->payment_method){
                        if($this->api_response_body->transaction->payment_method->token){
                            return true;
                        }
                    }
				}
